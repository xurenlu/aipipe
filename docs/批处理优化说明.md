...
```bash
# 每批最多 5 行
./supertail -f app.log --format java --batch-size 5

# 等待 1 秒后处理
./supertail -f app.log --format java --batch-wait 1s

# 组合使用
./supertail -f app.log --format java --batch-size 20 --batch-wait 5s
```
...
# 大批次（更节省，但延迟更高）
./supertail -f app.log --format java --batch-size 20
```
...
# 更高效（延迟大，但批次更大）
./supertail -f app.log --format java --batch-wait 10s
```
...
# 批处理模式
cat test.log | ./supertail --format java --verbose

# 逐行模式（对比）
cat test.log | ./supertail --format java --no-batch --verbose
```
...
```bash
# 每批 5 行，等待 10 秒
./supertail -f /var/log/app.log --format java --batch-size 5 --batch-wait 10s
```
...
```bash
# 大批次，快速处理
cat old.log | ./supertail --format java --batch-size 50 --batch-wait 5s
```
