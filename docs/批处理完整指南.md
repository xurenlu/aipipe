...
SuperTail 现在完全支持批处理模式，解决了"一行一发"的问题。
...
```bash
# 大批次，少调用
./supertail -f /var/log/app.log --format java --batch-size 20 --batch-wait 5s
```
...
```bash
# 小批次，快响应
./supertail -f /var/log/app.log --format java --batch-size 5 --batch-wait 1s
```
...
```bash
# 推荐配置
./supertail -f /var/log/production.log --format java --batch-size 15 --batch-wait 5s
```
...
```bash
# 快速响应配置
./supertail -f /var/log/dev.log --format java --batch-size 5 --batch-wait 1s --verbose
```
...
```bash
# 大批次配置
cat old.log | ./supertail --format java --batch-size 50 --batch-wait 10s
```
...
```bash
# 逐行模式
tail -f dev.log | ./supertail --format java --no-batch --verbose
```
...
# 监控文件
./supertail -f /tmp/test.log --format java --verbose
```
...
3. 重新编译确保使用最新版本：
   ```bash
   cd /Users/rocky/bin && go build -o supertail supertail.go
   ```
...
A: 减少批处理参数：
```bash
./supertail -f app.log --format java --batch-size 5 --batch-wait 1s
```
...
A: 增加批处理参数：
```bash
./supertail -f app.log --format java --batch-size 30 --batch-wait 10s
```
...
A: 使用 `--no-batch` 参数：
```bash
./supertail -f app.log --format java --no-batch
```
...
**现在可以愉快地使用 SuperTail 了，不会再"一行一发，有点受不了"！** 🎊
